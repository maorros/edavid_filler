...
scale = 2500.0
scale = 1625.0
with open('default.combine2.csv', 'rb') as csvfile:
    ...
    for row in reader:
        ...
        x_list.append(np.max([10.0,float(row['x'])*scale+170+100+250]))
        y_list.append(np.max([10.0,float(row['y'])*scale+100+100+150]))
        n += 1
        if n>200000:
            break
...
for k in range(int(0*len(x_list)),int(0.4*len(x_list)-1)):
